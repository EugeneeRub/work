...
import com.ForWork.lab11.WriteInFile;
        ...
        do {
            ...
            switch (mStr) {
                case "-finder":
                    ...
                    if (!mAutoFlag) {
                        FinderInText finder = new FinderInText(conteiner);
                        String regex = "(Пусть|Чтобы)? (любит|любила|любил) бегать (по|в)? (городу|городе|парку|парке)?";
                        finder.find(regex);
                    } else {
                        FinderInText finder = new FinderInText(conteiner);
                        String regex = "(Пусть|Чтобы)? (любит|любила|любил) бегать (по|в)? (городу|городе|парку|парке)?";
